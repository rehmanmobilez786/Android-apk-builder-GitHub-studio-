import { GitHubConfig, AndroidFile, GitHubRepoItem } from "../types";
...
export function ensureCompleteAndroidProjectFiles(currentFiles: AndroidFile[]): AndroidFile[] {
  ...
  currentFiles.forEach((f) => {
    if (f.path && f.content !== undefined) {
      // If user had an older version of index.html or truncated/incomplete version, upgrade to fresh rich version
      if (
        f.path === "index.html" &&
        (f.content.includes("1-CLICK APK") ||
          f.content.length < 1000 ||
          !f.content.includes("codeEditorArea") ||
          !f.content.includes("view-editor"))
      ) {
        const latestIndex = STARTER_PROJECT.files.find((sf) => sf.path === "index.html");
        if (latestIndex) {
          mergedMap.set("index.html", latestIndex);
          return;
        }
      }
      mergedMap.set(f.path, f);
...
/**
 * Fetches user repositories from GitHub using token or username
 */
export async function fetchUserGitHubRepos(
  token?: string,
  username: string = "rehmanmobilez786"
): Promise<{ success: boolean; repos: GitHubRepoItem[]; message?: string }> {
  try {
    ...
    // If token exists, fetch authenticated user repos first (includes private repos)
    let url = token && token.trim()
      ? "https://api.github.com/user/repos?sort=updated&per_page=100&type=all"
      : `https://api.github.com/users/${encodeURIComponent(username.trim() || "rehmanmobilez786")}/repos?sort=updated&per_page=100`;

    let res = await fetch(url, { headers });

    // Fallback if token failed or 404
    if (!res.ok && token && token.trim()) {
      const fallbackUrl = `https://api.github.com/users/${encodeURIComponent(username.trim() || "rehmanmobilez786")}/repos?sort=updated&per_page=100`;
      res = await fetch(fallbackUrl, {
        headers: { Accept: "application/vnd.github.v3+json", "User-Agent": "APK-Builder-Studio" },
      });
    }
    ...
    if (!res.ok) {
      ...
      return {
        success: false,
        repos: [],
        message: errData.message || `GitHub API error (${res.status})`,
      };
    }

    const data = await res.json();
    if (!Array.isArray(data)) {
      return { success: false, repos: [], message: "Invalid response format from GitHub." };
    }

    const repos: GitHubRepoItem[] = data.map((item: any) => ({
      id: item.id,
      name: item.name,
    ...
    return {
      success: false,
      repos: [],
      message: err.message || "Failed to fetch GitHub repositories.",
    };
  }
}

/**
 * Creates a brand new GitHub repository on the user's account
 */
export async function createNewGitHubRepo(
  token: string,
  repoName: string,
  isPrivate: boolean = false,
  description: string = "Android APK Studio Project"
): Promise<{ success: boolean; message: string; repo?: GitHubRepoItem }> {
  if (!token || !token.trim()) {
    return {
      success: false,
      message: "⚠️ ریپوزیٹری بنانے کے لیے Personal Access Token (PAT) درکار ہے۔",
    };
  }
  ...
  if (!cleanName) {
    return {
      success: false,
      message: "⚠️ درست ریپوزیٹری نام درج کریں۔",
    };
  }
  ...
  try {
    const res = await fetch("https://api.github.com/user/repos", {
      ...
      body: JSON.stringify({
        name: cleanName,
        description: description.trim() || "Android Studio project generated by APK Builder",
    ...
    if (res.status === 401) {
      return {
        success: false,
        message: "❌ غیر معتبر GitHub Token۔ براہ کرم 'repo' اسکوپ کے ساتھ نیا ٹوکن استعمال کریں۔",
      };
    }
    ...
    if (res.status === 422) {
      return {
        success: false,
        message: `⚠️ ریپوزیٹری '${cleanName}' پہلے سے موجود ہے۔ آپ پرانی ریپوزیٹری آپشن میں جا کر اسے منتخب کر سکتے ہیں۔`,
      };
    }
    ...
    if (!res.ok) {
      ...
      return {
        success: false,
        message: `❌ ریپو بنانے میں خرابی: ${err.message || res.statusText}`,
      };
    }
    ...
    return {
      success: true,
      message: `✅ نئی ریپوزیٹری '${data.full_name}' گٹ ہب پر کامیابی سے بن گئی!`,
      repo: {
        id: data.id,
        name: data.name,
    ...
    return {
      success: false,
      message: `❌ ایرر: ${err.message || String(err)}`,
    };
  }
}

/**
 * Pulls source code files from an existing repository
 */
export async function pullFilesFromGitHubRepo(
  token: string,
  owner: string,
  repo: string,
  branch: string = "main"
): Promise<{ success: boolean; files?: AndroidFile[]; message: string }> {
  const sanitized = sanitizeGitHubInputs(owner, repo);
  const cleanOwner = sanitized.owner;
  const cleanRepo = sanitized.repo;
  ...
  try {
    ...
    if (!refRes.ok) {
      return {
        success: false,
        message: `❌ برانچ '${cleanBranch}' نہیں مل سکی۔ ریپوزیٹری کا نام یا برانچ چیک کریں۔`,
      };
    }
    ...
    if (!treeRes.ok) {
      return {
        success: false,
        message: `❌ گٹ ہب ٹری حاصل نہیں ہو سکی: ${treeRes.statusText}`,
      };
    }
    ...
    // Fetch contents of up to 35 critical code files
    const relevantBlobs = blobs
      .filter((b: any) =>
        b.path.endsWith(".kt") ||
        b.path.endsWith(".java") ||
        b.path.endsWith(".xml") ||
        b.path.endsWith(".gradle") ||
        b.path.endsWith(".kts") ||
        b.path.endsWith(".properties") ||
        b.path.endsWith(".pro") ||
        b.path.endsWith(".md") ||
        b.path.endsWith(".yml") ||
        b.path.endsWith(".yaml")
      )
      .slice(0, 35);

    const loadedFiles: AndroidFile[] = [];

    for (const blob of relevantBlobs) {
      try {
        const fileRes = await fetch(
          `https://raw.githubusercontent.com/${cleanOwner}/${cleanRepo}/${cleanBranch}/${blob.path}`,
          token && token.trim() ? { headers: { Authorization: `Bearer ${token.trim()}` } } : {}
        );
        if (fileRes.ok) {
        ...
        console.warn(`Could not load file ${blob.path}`, e);
      }
    }
    ...
    if (loadedFiles.length === 0) {
      return {
        success: false,
        message: "ریپوزیٹری میں کوئی انڈرائیڈ سورس فائل نہیں ملی۔",
      };
    }
    ...
    return {
      success: true,
      files: loadedFiles,
      message: `✅ کامیابی! ${loadedFiles.length} سورس فائلیں '${cleanOwner}/${cleanRepo}' سے لوڈ ہو گئیں۔`,
    };
  } catch (err: any) {
    return {
      success: false,
      message: `❌ امپورٹ ایرر: ${err.message || String(err)}`,
    };
  }
}
